...
# Based the script from here:
# > Docs page:    https://docs.luxonis.com/software/depthai/manual-install/
# > Raw contents: https://raw.githubusercontent.com/luxonis/depthai-python/refs/heads/main/docs/install_dependencies.sh
# Adjust to only list and not install

trap 'RET=$? ; echo -e >&2 "\n\x1b[31mFailed listing dependencies. Could be a bug in the lister or unsupported platform. Open a bug report over at https://github.com/luxonis/depthai - exited with status $RET at line $LINENO \x1b[0m\n" ;
exit $RET' ERR
...
if [[ $(uname) == "Darwin" ]]; then
    echo "This script is not adapted for Darwin."
    ...
    if [ -f /etc/debian_version ]; then
        ...
        echo "Debian vresion = ${version}"
        echo "Is ARm flag    = ${IS_ARM}"

        if [ $IS_ARM ]; then
            echo ""
            echo "============================================"
            echo "NOW LISTING THE \"debian_arm_pkgs\""
            echo "============================================"
            echo ""

            #sudo DEBIAN_FRONTEND=noninteractive install -y list "${debian_arm_pkgs[@]}"
            apt list "${debian_arm_pkgs[@]}"

            if [[ $version == 13* ]]; then
                echo ""
                echo "============================================"
                echo "NOW LISTING THE \"debian_pkgs_23\""
                echo "============================================"
                echo ""

                echo "Detected ARM Debian 13"
                #sudo apt install -y "${debian_pkgs_23[@]}"
                apt list "${debian_pkgs_23[@]}"

            elif version_lte "$version" "11.99"; then
                echo ""
                echo "============================================"
                echo "NOW LISTING THE \"debian_pkgs_pre22_04\""
                echo "============================================"
                echo ""

                echo "Using pre-22.04 ARM package list"
                #sudo apt-get install -y ${debian_pkgs_pre22_04[@]}
                apt list ${debian_pkgs_pre22_04[@]}

                # Check for uvcdynctrl package and recommend removal if found
                if dpkg -s uvcdynctrl &> /dev/null; then
                    echo ""
                    echo "============================================"
                    echo "NOW RAISING \"uvcdynctrl\" WARNING"
                    echo "============================================"
                    echo ""
